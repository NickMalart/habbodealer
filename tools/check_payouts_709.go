...
func main() {
	...
	fmt.Println("--- auto_payouts for banker_trade_id 709 ---")
	rows, err := conn.Query(ctx, `SELECT id, player_name, item_name, quantity, status, created_at FROM auto_payouts WHERE banker_trade_id = 709`)
	if err != nil {
