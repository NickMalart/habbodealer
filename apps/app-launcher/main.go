	...
	return []buildTarget{
		{
			ID:        "roll-origins",
			Name:      "roll-origins",
			SrcDir:    filepath.Join("apps", "roll-origins"),
			BuildType: "wails",
			CleanPaths: []string{
				filepath.Join("apps", "roll-origins", "build", "bin", "roll-origins.exe"),
				filepath.Join("apps", "roll-origins", "build", "bin", "Gamba-Suite.exe"),
			},
			OutExe: filepath.Join("apps", "roll-origins", "build", "bin", "roll-origins.exe"),
		},
		{
			ID:        "wave-timer",
			Name:      "Wave Timer",
			SrcDir:    filepath.Join("apps", "wave-timer-app"),
			BuildType: "wails",
			CleanPaths: []string{
				filepath.Join("apps", "wave-timer-app", "build", "bin", "wave-timer-app.exe"),
				filepath.Join("apps", "wave-timer-app", "wave-timer-app.exe"),
				filepath.Join("apps", "wave-timer-app", "wave-timer.exe"),
			},
			OutExe: filepath.Join("apps", "wave-timer-app", "build", "bin", "wave-timer-app.exe"),
		},
		{
			ID:        "trade-tracker",
			Name:      "Trade Tracker",
			SrcDir:    filepath.Join("apps", "trade-tracker"),
			BuildType: "wails",
			CleanPaths: []string{
				filepath.Join("apps", "trade-tracker", "build", "bin", "trade-tracker.exe"),
				filepath.Join("apps", "trade-tracker", "trade-tracker.exe"),
			},
			OutExe: filepath.Join("apps", "trade-tracker", "build", "bin", "trade-tracker.exe"),
		},
		{
			ID:        "free-raffle-bot",
			Name:      "Free Raffle Bot",
			SrcDir:    filepath.Join("apps", "free-raffle-bot"),
			BuildType: "wails",
			CleanPaths: []string{
				filepath.Join("apps", "free-raffle-bot", "build", "bin", "free-raffle-bot.exe"),
				filepath.Join("apps", "free-raffle-bot", "free-raffle-bot.exe"),
			},
			OutExe: filepath.Join("apps", "free-raffle-bot", "build", "bin", "free-raffle-bot.exe"),
		},
	}
}
...
func (a *App) resolveWorkspaceRoot() string {
	...
	for _, c := range candidates {
		...
		for {
			...
			// workspace root has db.local.json and the apps/ directory
			if fileExists(filepath.Join(abs, "db.local.json")) && dirExists(filepath.Join(abs, "apps")) {
				return abs
			}
