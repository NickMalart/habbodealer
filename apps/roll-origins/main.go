...
func (a *App) initUsers28ParserCommand() {
	...
	if strings.TrimSpace(a.users28ParserScript) == "" {
		...
		scriptCandidates := []string{
			// Primary: exe is at apps/roll-origins/build/bin/ → four levels up = workspace root
			filepath.Join(exeDir, "..", "..", "..", "..", "internal", "scripts", "parse_users28.py"),
			// Fallback for go run / dev workflows (running from apps/roll-origins)
			filepath.Join("..", "..", "internal", "scripts", "parse_users28.py"),
		...
		if a.users28ParserScript == "" {
			...
			a.users28ParserScript = filepath.Join("internal", "scripts", "parse_users28.py")
		}
	}
...
func runUsers28PythonParser(a *App, packetData []byte) (*ParsedUsers28Result, error) {
	scriptPath := filepath.Join("internal", "scripts", "parse_users28.py")
	py := ""
