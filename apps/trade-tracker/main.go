...
func loadDBConfig() (*DBConfig, error) {
	...
	candidates = append(candidates, filepath.Join("apps", "trade-tracker", "db.local.json"))
...
func runUsers28PythonParser(packetData []byte) ([]ParsedUsers28User, error) {
	...
	scriptCandidates := []string{
		// Primary: exe is at apps/trade-tracker/build/bin/ → 4 levels up = workspace root
		filepath.Join(exeDir, "..", "..", "..", "..", "internal", "scripts", "parse_users28.py"),
		// Fallbacks for go run / dev workflows (running from apps/trade-tracker)
